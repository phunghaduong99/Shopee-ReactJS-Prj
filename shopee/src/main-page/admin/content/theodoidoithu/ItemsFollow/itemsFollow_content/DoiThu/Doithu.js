...
class Doithu extends Component {
    ...
    callApi = () => {
        if (this.props.listRivalsItem.length === 0) {
            axios({
                method: 'get',
                url: 'http://172.104.173.222:8081/getRivals/' + this.props.shopIdSelected + '/' + this.props.followingItemSelected,
                headers: {
        ...
        if (this.props.listRivalsShop.length === 0) {
            axios({
                method: 'get',
                url: 'http://172.104.173.222:8081/shopRival/' + this.props.shopIdSelected + '/' + this.props.followingItemSelected,
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `${this.props.token}`
                },
